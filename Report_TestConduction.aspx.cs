...
partial class Report_TestConduction : System.Web.UI.Page
{
    ...
    protected void BtnSearch_Click(object sender, System.EventArgs e)
    {
        ...
        if (ReportType == 1)
        {
            lblReportType.Text = "This report is generated for only those tests whose Attendance Authorisation is pending";
        }
        else if (ReportType == 2)
        {
            lblReportType.Text = "This report is generated for only those tests whose Attendance Authorisation is done and Marks Authorisation is pending";
        }
        else if (ReportType == 3)
        {
            lblReportType.Text = "'P' indicates that a Test is pending for Marks Authorisation in atleast 1 Batch at that centre.<br>'&#10004;' indicates that Test is Authorised in all batches at that centre.";
        }
        else if (ReportType == 4)
        {
            lblReportType.Text = "This report is generated for tests scheduled irrespective of Authorisation status";
        }
        ...
        string Report_Period = SearchPanel1.ReportPeriod.ToString();
        if (Report_Period != "")
        {
        ...
        DataSet dsGrid = ProductController.Report_Test_PendingAuthorisation(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code,FromDate,ToDate);
        int ColCnt = 0;
        try
        {
            foreach (DataColumn col in dsGrid.Tables[0].Columns)
            {
                ...
                if (dsGrid.Tables.Count > 1 && ReportType == 3)//this code is created because of Syllabus character legth is greater than 128 and Pivot is not display more than 128  character legth that is the reason
                {
    ...
    private void SetSearchPanel_UserControl()
    {
        ...
        SearchPanel1.FillDDL_ReportType(2);
    }
    ...
    protected void btnExport_Click(object sender, System.EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Report_TestConduct_" + lblReportType_Result.Text.Replace(" ","_") + ".xls");
