...
partial class Report_TestPerformance : System.Web.UI.Page
{
    ...
    protected void BtnSearch_Click(object sender, System.EventArgs e)
    {
        ...
        string BatchCode = SearchPanel1.Batch_Code;
        ...
        else if (Convert.ToInt32(SearchPanel1.ReportTypeCode) == 4)
        {
            ReportType = 8;
        }
        ...
        if (ReportType == 8)
        {
            Test_Id = SearchPanel1.Test_ID_Report_Type_8;
            Subject_Code = "All";
        }
        else
        {
        ...
        if (Test_Id == "Select" && ReportType == 8)
        {
        ...
        {
            DataSet dsGrid = null;
            if (string.IsNullOrEmpty(Report_Period) && ReportType == 7)
            {
                ToDate = "";
            }
            ...
            if (ReportType == 3 || ReportType == 4)
            {
                ...
                ddlMarksType_SelectedIndexChanged(sender, e);
                ControlVisibility("Result");
                return;
            }
            //Report_Test_MCQ_Ranking1
            dsGrid = ProductController.Report_Test_MCQ_Ranking1(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, BatchCode);
            //dsGrid = ProductController.Report_Test_MCQ_Ranking(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate);
            //DataSet dsGrid = ProductController.Report_Test_Performance(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate);
            int ColCnt = 0;
            try
            {                
        ...
        else
        {
            ...
            // DataSet dsGrid = ProductController.Report_Test_MCQ_Ranking_Subject(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code);
            //DataSet dsGrid = ProductController.Report_Test_Performance(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code);

            DataSet dsGridnew = null;
            ...
            {                
                ...
                dsGridnew = ProductController.Report_Test_MCQ_Ranking_Subject_Batch(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code,BatchCode);
                //ProductController.Report_Test_Performance(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code);
                // DataSet dsGrid = ProductController.Report_Test_MCQ_Ranking_Subject(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code);
                //int TotalRow = dsGridnew.Tables[0].Rows.Count;
            }
            else
            {
                dsGridnew = ProductController.Report_Test_MCQ_Ranking(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate,BatchCode);
            }
            ...
            try
            {
                if (dsGridnew != null)
                {
                    foreach (DataColumn col in dsGridnew.Tables[0].Columns)
                    {
                        //Declare the bound field and allocate memory for the bound field.
                        BoundField bfield = new BoundField();
                    ...
                    //Initialize the DataSource
                    dlGridReport1.DataSource = dsGridnew.Tables[0];
    ...
    protected void ddlMarksType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (Subject_Code == "All")
        {
            // DataSet dsGrid = ProductController.Report_Test_MCQ_Ranking(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, MarksType);
            DataSet dsGrid = null;
            if ((ReportType == 3 || ReportType == 4) && MarksType == 2)
            {
                dsGrid = ProductController.Report_Test_Performance(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code, MarksType);
            }
            else
            {
                dsGrid = ProductController.Report_Test_Performance(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code);
            }
        ...
        else
        {
            ...
            if (ReportType == 7)
            {
                NewdsGrid = ProductController.Report_Test_MCQ_Ranking_Subject(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, Subject_Code);
            }
            NewdsGrid = ProductController.Report_Test_Performance1(Test_Id, SearchPanel1.UserCode, ReportType, Centre_Code, FromDate, ToDate, MarksType);
            int ColCnt = 0;
            try
            {
                foreach (DataColumn col in NewdsGrid.Tables[0].Columns)
                {
                    //Declare the bound field and allocate memory for the bound field.
                    BoundField bfield = new BoundField();
                ...
                //Initialize the DataSource
                dlGridReport1.DataSource = NewdsGrid.Tables[0];
